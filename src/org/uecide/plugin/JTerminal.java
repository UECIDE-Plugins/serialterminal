...
public class JTerminal extends JComponent implements KeyListener,MouseListener,FocusListener,MouseMotionListener
{
    ...
    class ColorCharacter {
        ...
        int fontStyle;
        char letter;
        public ColorCharacter(char ch, int sty, Color fg, Color bg) {
            letter = ch;
            fontStyle = sty;
            fgColor = fg;
            bgColor = bg;
    ...
    class Line {
        ArrayList<ColorCharacter> characters = new ArrayList<ColorCharacter>();

        public ColorCharacter getCharAt(int i) {
            return characters.get(i);
        }

        public void setCharacterAt(int i, char c, int style, Color fg, Color bg) {
            ColorCharacter ch = new ColorCharacter(c, style, fg, bg);
            characters.set(i, ch);
        }

        public void setCharacterAt(int i, ColorCharacter c) {
            characters.set(i, c);
        }
    }

    class Screen {
        RStack<Line> lines = new RStack<Line>();
        Point cursor = new Point(0, 0);

        public ColorCharacter[] getScreenData(int topLine, int width, int height) {
            ColorCharacter[] out = new ColorCharacter[height * width];
            for (int y = 0; y < height; y++) {
                Line l = lines.get(topLine + y);
                if (l == null) {
...
//        repaint();
    ...
    public void loadCodePage(String file) {
        InputStream in;
        in = JTerminal.class.getResourceAsStream(file);
